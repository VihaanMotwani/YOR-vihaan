...
public class CameraStreamReceiver : MonoBehaviour
{
    ...
    private const bool CreateHeadLockedVideoSurface = true;
    private static readonly Vector3 ViewerLocalOffset = new Vector3(0f, -1.6f, 1f);
    private static readonly Vector3 VideoSurfaceLocalPosition = new Vector3(0f, 0f, 1.0f);
    private static readonly Vector3 VideoSurfaceLocalScale = new Vector3(2.1f, 1.18125f, 1f);
    ...
    private Renderer headLockedVideoRenderer;
    ...
    {
        if (headLockedVideoRenderer != null)
        {
            headLockedVideoRenderer.material.mainTexture = tex;
        }
    ...
    private void EnsureHeadLockedVideoSurface()
    {
        if (!CreateHeadLockedVideoSurface || headLockedVideoRenderer != null)
        {
            return;
        }
        ...
        if (mainCamera == null)
        {
            Debug.LogWarning("[CameraStreamReceiver] Could not create head-locked video surface because Camera.main was not found.");
            return;
        }

        GameObject surface = new GameObject("YOR Head-Locked Camera Stream");
        surface.name = "YOR Head-Locked Camera Stream";
        surface.transform.SetParent(mainCamera.transform, false);
        surface.transform.localPosition = VideoSurfaceLocalPosition;
        surface.transform.localRotation = Quaternion.identity;
        surface.transform.localScale = VideoSurfaceLocalScale;

        MeshFilter meshFilter = surface.AddComponent<MeshFilter>();
        meshFilter.mesh = CreateDoubleSidedQuadMesh();
        headLockedVideoRenderer = surface.AddComponent<MeshRenderer>();
        ...
        headLockedVideoRenderer.material = material;
        headLockedVideoRenderer.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
        headLockedVideoRenderer.receiveShadows = false;
        headLockedVideoRenderer.sortingOrder = 100;

        Debug.Log("[CameraStreamReceiver] Created head-locked video surface in front of Main Camera.");
    }

    private static Mesh CreateDoubleSidedQuadMesh()
    {
        Mesh mesh = new Mesh();
        mesh.name = "YOR Double-Sided Video Quad";
        mesh.vertices = new[]
        {
